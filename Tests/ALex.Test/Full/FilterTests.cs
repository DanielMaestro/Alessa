...
{
    public class FilterTests : ALexTesterFull
    {
        [Fact]
...
";
        ...
        [Fact]
        public void FilterRaw()
        {
            var query = @"From(My.Table)
Select(My.Col1, My.Col2)
Filter((My.Col1 IS NOT NULL OR My.Col < 0) )
Filter ((RAW('CONVERT(DATE, My.Col3) <= GETDATE()') OR R <> 4) AND (R > 345 OR My.Col7 IS NOT NULL))
Filter ((RAW('CONVERT(DATE, My.Col3) <= GETDATE()') AND R <> 4) OR (R > 345 AND My.Col7 IS NOT NULL))
";
            var q = this.GetQuery(query);
            Assert.Equal("SELECT [My].[Col1], [My].[Col2] FROM [My].[Table] WHERE (([My].[Col1] IS NOT NULL OR [My].[Col] <  0)) AND ((CONVERT(DATE, My.Col3) <= GETDATE() OR [R] <>  4) AND ([R] >  345 OR [My].[Col7] IS NOT NULL)) AND ((CONVERT(DATE, My.Col3) <= GETDATE() AND [R] <>  4) OR ([R] >  345 AND [My].[Col7] IS NOT NULL))", base.GetSentence(q));
        }
    }
}
