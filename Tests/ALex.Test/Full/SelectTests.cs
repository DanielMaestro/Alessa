...
{
    public class SelectTests : ALexTesterFull
    {
        [Fact]
...
";
        ...
        {
            var query = @"From(My.Table AS My)
Select(My.Col1 AS ThisColum, My.Col2 AS AnotherColumn, My.Col3 AS 'String.ColumName')
";
            var q = this.GetQuery(query);
            Assert.Equal(@"SELECT [My].[Col1] AS [ThisColum], [My].[Col2] AS [AnotherColumn], [My].[Col3] AS [String.ColumName] FROM [My].[Table] AS [My]", base.GetSentence(q));
        }
